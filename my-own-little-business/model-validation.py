...
if __name__ == "__main__":
    ...
    conn = create_engine(dsn, echo=False)
    ...
    with conn.begin() as trans:
        ...
        product_1_id = result.fetchone()[0]
        ...
        product_2_id = result.fetchone()[0]

        # batch_1
        result = conn.execute(
           insert(Batch).values(
               date="2017-06-18 08:00:00", capacity=50
            ).returning(Batch.__table__.c.id)
        )
        batch_1_id = result.fetchone()[0]

        # batch_2
        result = conn.execute(
            insert(Batch).values(
                date="2017-06-19 08:00:00", capacity=50, opened=False
            ).returning(Batch.__table__.c.id)
        )
        batch_2_id = result.fetchone()[0]
    ...
    conn.execute(update(Client).where(
    ...
    result = conn.execute(
        insert(Payment).values(
            total=10, payed_at=datetime.datetime.now(), mode=PayedStatusEnum.payed_by_check
        ).returning(Payment.__table__.c.id)
    )
    payement_2_id = result.fetchone()[0]
